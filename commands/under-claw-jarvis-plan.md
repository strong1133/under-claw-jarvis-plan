...
- **작업자/검수자 분리**: 목표와 결과물에 대한 검증·검수·리뷰는 작업한 agent가 아닌 다른 agent가 맡는다. 3-pane에서는 작업자를 제외한 두 agent가 가능하면 모두 검수한다.
...
> 3-pane/council 단계의 DoD에는 단계별 합의 게이트 상태표(독립산출/교차검토/유효ACK 전 칸 충족), 진행 막는 `[BLOCK]` 없음, 가능한 경우 draft mtime과 `[DRAFT_READY]` 전송시각 점검 결과가 포함된다.
