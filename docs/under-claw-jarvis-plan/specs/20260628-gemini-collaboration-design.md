...
> 역사적 설계 기록. 현재 합의 게이트는 고정 3모델 시뮬레이션이 아니라 단계 시작 시 실제로 확정한
> `ACTIVE_PARTICIPANTS`만 인정한다. 아래의 가상 서명/self-verification 방식은 더 이상 유효한 완료 증거가 아니다.
...
- **당시 완화책(폐기됨)**: GEMINI가 CLAUDE, CODEX 관점을 시뮬레이션했다. 현재 규칙에서는 역할 패스로만 기록하며 실제 타 모델의 ACK로 계상하지 않는다.

## 6. 검증 방법
- **테스트 방법**: 자비스 플랜 `references/90-test.md`에 명시된 4대 매트릭스 리포트를 정교하게 작성하여 self-verification 수행 및 PASS 판정 실증.

## 7. task 분할
- **Task 1**: 설계 문서 생성 및 반영 (`docs/under-claw-jarvis-plan/specs/20260628-gemini-collaboration-design.md`) - [작업: GEMINI, 검수: 당시 시뮬레이션]
