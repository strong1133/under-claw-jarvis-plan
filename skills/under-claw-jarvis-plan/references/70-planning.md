...
2. **유저 전역**: `~/.claude/under-claw-jarvis-plan.skillmap.md`

둘 다 있으면 **프로젝트가 우선**(전역은 기본값), 키별로 병합. 없으면 `60`의 유형 맵만으로 동작(graceful).
...
3. 맵에 적힌 스킬이 런타임에 없으면 **⏭️ + 사유 1줄** 후 council/서브에이전트로 대체(Karpathy 4).
